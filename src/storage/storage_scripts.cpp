...
namespace OpenWifi {
	static ORM::FieldVec ScriptDB_Fields{ORM::Field{"id", 36, true},
	...
	static ORM::IndexVec MakeIndices(const std::string &shortname) {
		return ORM::IndexVec{{std::string(shortname + "_name_index"),
							  ORM::IndexEntryVec{{std::string("name"), ORM::Indextype::ASC}}}};
	};

	ScriptDB::ScriptDB(const std::string &TableName, const std::string &Shortname,
					   OpenWifi::DBType T, Poco::Data::SessionPool &P, Poco::Logger &L)
		: DB(T, TableName.c_str(), ScriptDB_Fields, MakeIndices(Shortname), P, L,
			 Shortname.c_str()) {}
	...
	void ScriptDB::Initialize() {
		...
		SE.uri = "https://telecominfraproject.atlassian.net/wiki/spaces/WIFI/pages/1678147585/"
				 "AP+Support+Bundle+ASB";
		SE.version = "1.0";
		SE.author = "blogic";
...
template <>
void ORM::DB<OpenWifi::ScriptRecordTuple, OpenWifi::GWObjects::ScriptEntry>::Convert(
	const OpenWifi::ScriptRecordTuple &In, OpenWifi::GWObjects::ScriptEntry &Out) {
	Out.id = In.get<0>();
	Out.name = In.get<1>();
...
template <>
void ORM::DB<OpenWifi::ScriptRecordTuple, OpenWifi::GWObjects::ScriptEntry>::Convert(
	const OpenWifi::GWObjects::ScriptEntry &In, OpenWifi::ScriptRecordTuple &Out) {
	Out.set<0>(In.id);
	Out.set<1>(In.name);
