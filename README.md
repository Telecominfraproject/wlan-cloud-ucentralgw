...
4. The deployment builds a new `ucentralgw` image with the `master` tag. If you want to change either that or some of the pre-built image versions which are used for the other services, have a look into the `.env` file. You'll also find service specific `.env` files in this directory. Edit them if you want to change database passwords (highly recommended!) or some other configuration data. Don't forget to adapt your changes in the application configuration files.
